...
class Person{
    ...
    //Setter
    void setPerson(string n,int a,string i,long long c){
        setName(n);
        setAge(a);       
        id=i;
        contact=c;
...
class Professor:public Person{
   ...
   Professor(string n,int a,string i,long long c,string s,string h,int sal)
    ...
    //Setter
    void setProfessor(string n,int a,string i,long long c,string s,string h,int sal){
        name=n;
        age=a;
        id=i;
        contact=c;
   ...
   Person::displayDetails();
    cout<<"specialisation:"<<specialisation<<endl<<"hire date:"<<hire_date<<endl;
   }
  ...
  return 10000;
   ...
   // Destructor
   ~Professor(){
    cout<<"Destructor called for Professor with specialization: "<<specialisation<<endl;
}
};
class Department{
    public:
    ...
    //Setter
    void setDepartment(string n,string l,double b){
        named=n;
        location=l;
        budget=b;
    }
    ...
    void displaydep(){
        ...
        if (!professors.empty()) {
            ...
            for (const auto& prof : professors) {
                cout << " - " << prof.name << ", Specialization: " << prof.specialisation << endl;}
            }
...
class Course{
    ...
    //Setter
    void setCourse(string co,string t,long long cr,string d,Professor* instr = nullptr){
        code=co;
        title=t;
        setCredit(cr);
        description=d;
...
class Student:public Person{
    ...
    //Setter
    void setStudent(string n,int a,string i,long long c,string e,string p,double g){
        name=n;
        age=a;
        id=i;
        contact=c;
...
class UniversitySystemException:public exception {
    protected:
        string errorMessage;
    public:
        UniversitySystemException(const string& message) : errorMessage(message) {}
    ...
    private:
        string courseCode;
    public:
        EnrollmentException(const string& message, const string& course) 
        : UniversitySystemException(message), courseCode(course) {}
    ...
    private:
        string studentID;
    public:
        GradeException(const string& message, const string& student) : UniversitySystemException(message), studentID(student) {}
    ...
    private:
        string personID;
    public:
        PaymentException(const string& message, const string& person) : UniversitySystemException(message), personID(person) {}
        string getPersonID() const { return personID; }
    };

    // Logging Mechanism
    void logException(const string& logMessage) {
        ofstream logFile("university_system.log", ios::app);
        if (logFile.is_open()) {
    ...
    // Person Exception
class PerException : public UniversitySystemException {
    private:
        string personID;
    public:
        PerException(const string& message, const string& person) : UniversitySystemException(message), personID(person) {}
        string getPersonID() const { return personID; }
    };
    // Person Class
class Per {
    ...
    public:
        void setID(const string& newID) {
            if (newID.empty()) {
                throw PerException("Invalid ID: ID cannot be empty.", newID);
            }
            id = newID;
        }
        ...
        void setContactInfo(const string& newContactInfo) {
            if (newContactInfo.empty()) {
                throw PerException("Invalid contact information: Cannot be empty.", id);
            }
            contactInfo = newContactInfo;
        }
    
        double calculatePayment(double baseSalary, int hoursWorked) {
            if (hoursWorked < 0) {
                throw PaymentException("Payment error: Hours worked cannot be negative.", id);
            }
...
class UniversitySystem {
    public:
        void menu() {
            ...
            while (true) {
                ...
                cout << "3. Assign Professor" << endl;
                cout << "4. Enter Grade" << endl;
                cout << "5. Finalize Grades" << endl;
                cout << "6. Calculate Payment" << endl;
                cout << "7. Exit" << endl;
                cout << "Enter choice: ";
                cin >> choice;
                ...
                try {
                    switch (choice) {
                        case 1:
                            cout << "Creating new semester..." << endl;
                            break;
                        case 2:
                            throw EnrollmentException("Enrollment error: Demo error.", "CS101");
                            break;
                        case 3:
                            cout << "Assigning professor..." << endl;
                            break;
                        case 4:
                            throw GradeException("Grade error: Demo error.", "24CSU101");
                            break;
                        case 5:
                            cout << "Finalizing grades..." << endl;
                            break;
                        case 6:
                            throw PaymentException("Payment error: Demo error.", "24CSU214");
                            break;
                        case 7:
                            cout << "Exiting..." << endl;
                            return;
    ...
    Person p1;
    Person p2;
    p1.setPerson("",18,"24csu214",99343468201);
    p1.displayDetails();
    p2.setPerson("tanisha",18,"24csu",8700732768);
    p2.displayDetails();
    cout<<endl;
    Student s1;
    Student s2;
    s1.setStudent("tanisha",18,"24csu214",9959758201,"12/03/2024","btech",4);
    s1.displayDetails();
    s2.setStudent("tanu",17,"24csu215",9950600000,"14/03/2025","bba",5);
    s2.displayDetails();
    cout<<endl;
    Professor pr1;
    Professor pr2;
    pr1.setProfessor("tanisha",35,"24tec45",943583475,"ai/ml","21/03/2024",10000);
    pr1.displayDetails();
    pr2.setProfessor("tanu",32,"24tec75",94465785,"data science","25/03/2025",30000);
    pr2.displayDetails();
    cout<<endl;
    Course c1;
    Course c2;
    c1.setCourse("XYZ","abc",5,"abcd");
    c1.displaycourse();
    c2.setCourse("ABC","xyz",4,"ab");
    c2.displaycourse();
    cout<<endl;
    Department d1;
    Department d2;
    d1.setDepartment("cse","first floor",5000000);
    d1.displaydep();
    d2.setDepartment("cse","second floor",30000);
    d2.displaydep();
    ...
    //test implimentation of custom exceptions
    try{
        throw EnrollmentException("Enrollment error: Course not found.", "CS101");
    } catch(EnrollmentException& e) {
        cout<<e.what()<<"-Course:"<<e.getCourseCode()<<endl;
    }

    try{
        throw GradeException("Grade error: Invalid grade value.","24CSU101");
    } catch (GradeException& e) {
        cout<<e.what()<<"- Student: "<<e.getStudentID()<<endl;
    }

    try{
        throw PaymentException("Payment error: Insufficient funds.", "24CSU214");
    } catch (PaymentException& e) {
        cout<<e.what()<<"-Person:"<<e.getPersonID()<<endl;
    }
